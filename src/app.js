...
import {
  pickRandomCardDefinitions,
  pickRandomCards,
} from "./model/card-util.js";
...
import {
  FOLD_CARDS_POLICY,
  HandComponent,
  MAX_SELECTION_REACHED_POLICY,
} from "./card-game/hand-component.js";
...
const maxCards = element.attributes?.maxCards?.value
  ? parseInt(element.attributes.maxCards.value)
  : 7;
...
const maxCardsReachedPolicy =
  element.attributes?.maxCardsReachedPolicy?.value ??
  MAX_SELECTION_REACHED_POLICY.BLOCK;
const foldCardsPolicy =
  element.attributes?.foldCardsPolicy?.value ??
  FOLD_CARDS_POLICY.AFTER_ANIMATION;
...
const properties = {
  ...
  maxSelectedCards,
  getLayoutConfiguration: (elementRef) =>
    new MediaConfiguration(elementRef, PLATFORM_CONFIGURATIONS),
};
